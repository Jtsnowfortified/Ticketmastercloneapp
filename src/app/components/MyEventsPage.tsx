...
import { fetchEventsWithTicketCounts } from '../services/supabaseClient';

/* ------------------------------------------------------------------ */
/*  Fallback hardcoded data (used when Supabase is not connected)      */
/* ------------------------------------------------------------------ */
...
export default function MyEventsPage() {
  ...
  const { data: dbEvents } = useSWR('events-with-counts', fetchEventsWithTicketCounts, {
    refreshInterval: 30000,
    revalidateOnFocus: true,
    fallbackData: null,
  });

  const upcomingEvents = (dbEvents || fallbackEvents).map((e) => ({
    id: e.id,
    title: e.title,
    date: e.date,
    venue: e.venue,
  ...
  return (
    <div className="min-h-screen bg-tm-bg flex flex-col">
      ...
      <div className="bg-tm-surface border-b border-tm-border">
        <div className="flex">
          <button onClick={() => setActiveTab('upcoming')} className="flex-1 relative py-3 text-center">
            <span className={`text-base font-semibold ${activeTab === 'upcoming' ? 'text-tm-text-primary' : 'text-tm-text-muted'}`}>
              Upcoming
            </span>
            {activeTab === 'upcoming' && <div className="absolute bottom-0 left-0 right-0 h-[3px] rounded-t-full bg-tm-blue" />}
          </button>
          <button onClick={() => setActiveTab('past')} className="flex-1 relative py-3 text-center">
            <span className={`text-base font-semibold ${activeTab === 'past' ? 'text-tm-text-primary' : 'text-tm-text-muted'}`}>
              Past
            </span>
            {activeTab === 'past' && <div className="absolute bottom-0 left-0 right-0 h-[3px] rounded-t-full bg-tm-blue" />}
          </button>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto pb-24 bg-tm-bg">
        {events.length > 0 ? (
              ...
              >
                <div className="relative" style={{ height: '260px' }}>
                  ...
                  <div className="absolute bottom-0 left-0 right-0 px-4 pb-4">
                    <h3 className="text-white text-lg font-bold leading-snug mb-1">{event.title}</h3>
          ...
          <div className="flex flex-col items-center justify-center h-64 text-center px-6">
            ...
            <p className="text-tm-text-muted text-sm mt-1">Events you have attended will show up here</p>
          </div>
        )}
      </div>
    </div>
